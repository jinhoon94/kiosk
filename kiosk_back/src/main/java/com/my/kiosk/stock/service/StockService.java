...
public class StockService {
	...
	// 발주state 처리할때..
	// 발주state가 기존에 있다면 기존의 발주 state를 변경해야할 필요가 있음. 가장 최근에 동일한 조건의 출고 내역을 확인해서 내역이
	// 있는지 확인.
	// 있다 -> 재발주, 기존 StockOut의 id로 state를 select해서, 새로 만든 StockOut id로 update하고
	// state 0으로 바꿈.
	// 출고 처리
	...
	public boolean verifyCaptcha(String captchaToken) {
		String secretKey = "6LfyKCArAAAAAAfnPEBRCAOIgYhoJWjLfGBiBQKD"; // 캡챠
																		// 비밀키!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
